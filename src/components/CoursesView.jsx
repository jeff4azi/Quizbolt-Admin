...
export default function CoursesView({ onNavigateToQuestions }) {
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      ...
      setNotification({ type: "success", text: `Course ${data.course_code} created successfully!` });
  ...
  return (
    <div className="p-6 space-y-6 text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">
            Browse and manage all registered course codes, level & semester metadata across institutions.
          </p>
        </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-400">Loading course metadata...</div>
        ) : courses.length === 0 ? (
          <div className="p-12 text-center text-slate-500 text-xs">No course records found.</div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-800/60">
                {courses.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/40 transition">
                    <td className="py-3.5 px-4 font-bold text-indigo-400">{c.course_code}</td>
                    <td className="py-3.5 px-4 font-semibold text-slate-300">{c.university}</td>
                    <td className="py-3.5 px-4 font-medium text-white">{c.title}</td>
                    <td className="py-3.5 px-4">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-slate-300 border border-slate-700 uppercase">
                        {c.course_group}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-400 font-mono text-[11px]">{Array.isArray(c.colleges) ? c.colleges.join(", ") : "ALL"}</td>
                    <td className="py-3.5 px-4 text-purple-300 font-semibold">{c.level}L / Sem {c.semester}</td>
                    <td className="py-3.5 px-4 text-right">
                      <button
                        onClick={() => onNavigateToQuestions && onNavigateToQuestions(c.course_code)}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block text-slate-400 mb-1 font-semibold">Full Course Title</label>
                <input
                  type="text"
                  required
